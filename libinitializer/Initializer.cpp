...
void Initializer::init_with_groupP2PService(std::string const& _path, std::shared_ptr<Service> _p2pService)
{
    try
    {
        ...
        m_ledgerInitializer->initConfig(pt, _p2pService);
...
void Initializer::init_with_groupP2PService(std::string const& _path, std::shared_ptr<Service> _p2pService, std::shared_ptr<Service> intra_p2pService, PROTOCOL_ID _group_protocolID)
{
    try
    {
        ...
        INITIALIZER_LOG(INFO) << LOG_DESC("tanghaibo进入Initializer::init_with_groupP2PService(std::string const& _path, std::shared_ptr<Service> _p2pService)");
        // m_ledgerInitializer->initConfig(pt);
        // m_ledgerInitializer->initConfig(pt, _p2pService);
        m_ledgerInitializer->initConfig(pt, _p2pService, _group_protocolID);
...
void Initializer::init(std::string const& _path)
{
    try
    {
        ...
        INITIALIZER_LOG(INFO) << LOG_DESC("tanghaibo进入Initializer::init(std::string const& _path)");
        m_ledgerInitializer->initConfig(pt);
