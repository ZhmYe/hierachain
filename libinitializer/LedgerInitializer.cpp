...
void LedgerInitializer::initConfig(boost::property_tree::ptree const& _pt)
{
    ...
    INITIALIZER_LOG(INFO) << LOG_DESC("正在LedgerInitializer::initConfig");
    initLedgers();
}

// 接收跨分片交易的通信群组的节点初始化代码
void LedgerInitializer::initConfig(boost::property_tree::ptree const& _pt, std::shared_ptr<Service> _p2pService)
{
    m_groupDataDir = _pt.get<string>("group.group_data_path", "data/");
    m_groupConfigPath = _pt.get<string>("group.group_config_path", "conf/");
    m_ledgerManager = make_shared<LedgerManager>();
...
bool LedgerInitializer::initLedgerByGroupID(dev::GROUP_ID const& _groupId)
{
    ...
    // INITIALIZER_LOG(INFO) << LOG_DESC( "进入 initLedgerByGroupID");
...
vector<dev::GROUP_ID> LedgerInitializer::initLedgers(std::shared_ptr<Service> _p2pService)
{
    vector<dev::GROUP_ID> newGroupIDList;
    try
    {
...
vector<dev::GROUP_ID> LedgerInitializer::initLedgers(std::shared_ptr<Service> _p2pService, PROTOCOL_ID _group_protocolID)
{
    vector<dev::GROUP_ID> newGroupIDList;
    try
    {
...
bool LedgerInitializer::initLedger(
    dev::GROUP_ID const& _groupId, std::shared_ptr<Service> _p2pService, std::string const& _dataDir, std::string const& _configFileName)
{
    if (m_ledgerManager->isLedgerExist(_groupId))
    {
        // Already initialized
    ...
    auto ledgerParams = std::make_shared<LedgerParam>();
    ledgerParams->init(_configFileName, _dataDir);
    bool succ = ledger->initLedger(ledgerParams, _p2pService);
    if (!succ)
    {
        return false;
    }
...
bool LedgerInitializer::initLedger(
    dev::GROUP_ID const& _groupId, std::shared_ptr<Service> _p2pService, PROTOCOL_ID _group_protocolID, std::string const& _dataDir, std::string const& _configFileName)
{
    if (m_ledgerManager->isLedgerExist(_groupId))
    {
        // Already initialized
