...
bool Ledger::initLedger(std::shared_ptr<LedgerParamInterface> _ledgerParams, std::shared_ptr<Service> _p2pService)
{
    BOOST_LOG_SCOPED_THREAD_ATTR(
        "GroupId", boost::log::attributes::constant<std::string>(std::to_string(m_groupId)));
    if (!_ledgerParams)
    ...
    initQPSLimit();
    /// init blockVerifier, txPool, sync and consensus
    return (initBlockVerifier() && initTxPool() && initSync() && consensusInitFactory(_p2pService) &&
            initEventLogFilterManager());
}


bool Ledger::initLedger(std::shared_ptr<LedgerParamInterface> _ledgerParams, std::shared_ptr<Service> _p2pService, PROTOCOL_ID _group_protocolID)
{
    BOOST_LOG_SCOPED_THREAD_ATTR(
        "GroupId", boost::log::attributes::constant<std::string>(std::to_string(m_groupId)));
    if (!_ledgerParams)
...
ConsensusInterface::Ptr Ledger::createConsensusEngine(dev::PROTOCOL_ID const& _protocolId, std::shared_ptr<Service> _p2pService)
{
    if (dev::stringCmpIgnoreCase(
            m_param->mutableConsensusParam().consensusType, PBFT_CONSENSUS_TYPE) == 0)
    {
        Ledger_LOG(INFO) << LOG_DESC("createConsensusEngine: create PBFTEngine");
        return std::make_shared<PBFTEngine>(m_service, m_txPool, m_blockChain, m_sync, 
            m_blockVerifier, _protocolId, m_keyPair, _p2pService, m_param->mutableConsensusParam().sealerList);
    }
    if (normalrPBFTEnabled())
    {
        Ledger_LOG(INFO) << LOG_DESC("createConsensusEngine: create RotatingPBFTEngine");
...
ConsensusInterface::Ptr Ledger::createConsensusEngine(dev::PROTOCOL_ID const& _protocolId, std::shared_ptr<Service> _p2pService, dev::PROTOCOL_ID _group_protocolId)
{
    if (dev::stringCmpIgnoreCase(
            m_param->mutableConsensusParam().consensusType, PBFT_CONSENSUS_TYPE) == 0)
    {
        ...
        Ledger_LOG(INFO) << LOG_DESC("正在 Ledger::createConsensusEngine(dev::PROTOCOL_ID const& _protocolId, std::shared_ptr<Service> _p2pService, dev::PROTOCOL_ID _group_protocolId)")
                         << LOG_KV("_group_protocolId", _group_protocolId);

        return std::make_shared<PBFTEngine>(m_service, m_txPool, m_blockChain, m_sync, 
            m_blockVerifier, _protocolId, m_keyPair, _p2pService, _group_protocolId, m_param->mutableConsensusParam().sealerList);
    }
    if (normalrPBFTEnabled())
    {
        Ledger_LOG(INFO) << LOG_DESC("createConsensusEngine: create RotatingPBFTEngine");
...
std::shared_ptr<Sealer> Ledger::createPBFTSealer()
{
    ...
    Ledger_LOG(INFO) << LOG_DESC("片内共识协议号") << LOG_KV("protocol_id", protocol_id);
 ...
 * @return std::shared_ptr<ConsensusInterface>: created consensus engine
 */
std::shared_ptr<Sealer> Ledger::createPBFTSealer(std::shared_ptr<Service> _p2pService)
{
    Ledger_LOG(INFO) << LOG_BADGE("initLedger") << LOG_BADGE("createPBFTSealer");
    if (!m_txPool || !m_blockChain || !m_sync || !m_blockVerifier || !m_dbInitializer)
    {
    ...
    ConsensusInterface::Ptr pbftEngine = createConsensusEngine(protocol_id, _p2pService);
    if (!pbftEngine)
    {
        BOOST_THROW_EXCEPTION(dev::InitLedgerConfigFailed() << errinfo_comment(
                                  "create PBFTEngine failed, maybe unsupported consensus type " +
 ...
 * @return std::shared_ptr<ConsensusInterface>: created consensus engine
 */
std::shared_ptr<Sealer> Ledger::createPBFTSealer(std::shared_ptr<Service> _p2pService, PROTOCOL_ID _group_protocolID)
{
    Ledger_LOG(INFO) << LOG_BADGE("initLedger") << LOG_BADGE("createPBFTSealer");
    if (!m_txPool || !m_blockChain || !m_sync || !m_blockVerifier || !m_dbInitializer)
    {
...
dev::eth::BlockFactory::Ptr Ledger::createBlockFactory(std::shared_ptr<Service> _p2pService)
{
    if (!m_param->mutableConsensusParam().enablePrepareWithTxsHash)
    {
        return std::make_shared<dev::eth::BlockFactory>();
...
/// init consensus
bool Ledger::consensusInitFactory(std::shared_ptr<Service> _p2pService)
{
    Ledger_LOG(INFO) << LOG_BADGE("initLedger") << LOG_BADGE("consensusInitFactory");
    // create RaftSealer
    if (dev::stringCmpIgnoreCase(
    ...
    // create PBFTSealer
             ...
             normalrPBFTEnabled() || vrfBasedrPBFTEnabled())
    {
        m_sealer = createPBFTSealer(_p2pService);
    }
    else
    {
        BOOST_THROW_EXCEPTION(
...
/// init consensus
bool Ledger::consensusInitFactory(std::shared_ptr<Service> _p2pService, PROTOCOL_ID _group_protocolID)
{
    Ledger_LOG(INFO) << LOG_BADGE("initLedger") << LOG_BADGE("consensusInitFactory");
    // create RaftSealer
    if (dev::stringCmpIgnoreCase(
