...
public:
    ...
    {
        Ledger_LOG(INFO) << LOG_DESC("开始初始化ledger...");
        assert(m_service);
    }
    ...
    bool initLedger(std::shared_ptr<LedgerParamInterface> _ledgerParams, std::shared_ptr<Service> _p2pService) override;
    bool initLedger(std::shared_ptr<LedgerParamInterface> _ledgerParams, std::shared_ptr<Service> _p2pService, PROTOCOL_ID _group_protocolID) override;
...
protected:
    ...
    /// create consensus moudle
    virtual bool consensusInitFactory(std::shared_ptr<Service> _p2pService);
    virtual bool consensusInitFactory(std::shared_ptr<Service> _p2pService, PROTOCOL_ID _group_protocolID);
    ...
    dev::consensus::ConsensusInterface::Ptr createConsensusEngine(dev::PROTOCOL_ID const& _protocolId);
    dev::consensus::ConsensusInterface::Ptr createConsensusEngine(dev::PROTOCOL_ID const& _protocolId, std::shared_ptr<Service> _p2pService);
    dev::consensus::ConsensusInterface::Ptr createConsensusEngine(dev::PROTOCOL_ID const& _protocolId, std::shared_ptr<Service> _p2pService, dev::PROTOCOL_ID _group_protocolId);

    dev::eth::BlockFactory::Ptr createBlockFactory();
    dev::eth::BlockFactory::Ptr createBlockFactory(std::shared_ptr<Service> _p2pService);
...
private:
    ...
    std::shared_ptr<dev::consensus::Sealer> createPBFTSealer(std::shared_ptr<Service> _p2pService);
    std::shared_ptr<dev::consensus::Sealer> createPBFTSealer(std::shared_ptr<Service> _p2pService, PROTOCOL_ID _group_protocolID);
