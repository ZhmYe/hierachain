...
void SyncBlockForStoragePacket::encode(dev::bytes const& _blockRLP)
{
    m_rlpStream.clear();
    prep(m_rlpStream, BlockForStoragePacket, 1);
    m_rlpStream.append(_blockRLP);
}

void SyncDistributedTxPacket::encode(dev::bytes const& _blockRLP)
{
    m_rlpStream.clear();
    prep(m_rlpStream, DistributedTxPacket, 1);
    m_rlpStream.append(_blockRLP);
}

void SyncPreCommittedTxPacket::encode(dev::bytes const& _blockRLP)
{
    m_rlpStream.clear();
    prep(m_rlpStream, PreCommittedTxPacket, 1);
    m_rlpStream.append(_blockRLP);
}

void SyncCommittedTxPacket::encode(dev::bytes const& _blockRLP)
{
    m_rlpStream.clear();
