...
std::string Rpc::sendRawTransaction(int _groupID, const std::string& _rlp, int cross_tx)
{
    if(cross_tx == 1)
    {
        std::this_thread::sleep_for(std::chrono::milliseconds(100)); // 暂停100毫秒，模拟跨分片通信
    }
...
//         auto txHash = tx->hash();
//         RPC_LOG(INFO) << LOG_DESC("分片拿到的跨片子交易")
//                         << LOG_KV("signedData", _rlp)
//                         << LOG_KV("txHash", txHash);
...
//     else
//     {
//         std::string return_value = "Invalid Third Param";
//         return return_value;
//     }
// }
