...
public:
    ...
    ConsensusEngineBase(std::shared_ptr<dev::p2p::P2PInterface> _service,
      ...
      : Worker("ConsensusEngine", 0),
        ...
        m_keyPair(_keyPair),
        m_group_service(_p2pService),
        m_group_protocolID(_group_protocolID),
        m_sealerList(_sealerList)
    {
        assert(m_service && m_txPool && m_blockChain && m_blockSync && m_blockVerifier);
        if (m_protocolId == 0)
...
protected:
    ...
    /// handler of the group_communication module, ADD BY THB
    std::shared_ptr<dev::p2p::Service> m_group_service;

    /// protocol ID ADD BY THB
