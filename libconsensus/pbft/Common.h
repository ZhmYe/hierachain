...
namespace consensus
{
    ...
    extern std::vector<std::pair<std::string, uint64_t>> count_committxTime; // 统计结束共识时间
    extern std::vector<std::pair<std::string, uint64_t>> count_starttxTime; //统计开始共识时间
...
struct PBFTMsgPacket
{
    ...
    virtual void streamRLPFields(RLPStream& s) const
    {
        ...
        s << packet_id << ttl << data << test_label;
    }
    ...
    virtual void populate(RLP const& rlp)
    {
        try
        {
            ...
            test_label = rlp[field = 3].toInt<uint8_t>(); //新增字段
        }
