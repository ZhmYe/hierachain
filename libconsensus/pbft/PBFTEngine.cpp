...
string PBFTEngine::dataToHexString(bytes data){
    string res2 = "";
    string temp;
    stringstream ioss;
...
bool PBFTEngine::generatePrepare(dev::eth::Block::Ptr _block)
{
    ...
    // // 开始统计交易延时(先假设所有交易都为片内交易)
    // for(auto ele:*_block->transactions()){
    //     // 判断交易类型, 若交易是原始跨片交易或者片内交易，那么开始计时
	//     string data_str = dataToHexString(ele->get_data());
    //     int index = -1;

    //     if((index = data_str.find("0x111222333", 0)) != -1){ // 原始跨片交易
    ...
    //         // count_starttxTime.push_back(std::make_pair(txid, utcSteadyTime()));
    //     }

    //     else if((index = data_str.find("0x444555666", 0)) != -1){ // 原始片内交易
    //         vector<string> dataItems;
    //         boost::split(dataItems, data_str, boost::is_any_of("|"), boost::token_compress_on);
